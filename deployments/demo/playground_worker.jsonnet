...

{
  ...
  schedulerGrpcClient: {
    address: 'unix://%s/playground_scheduler_workers.sock' % statePath,
  },
  filePool: { blockDevice: { file: {
    path: statePath + '/playground_worker_filepool',
    sizeBytes: 1e9,
  } } },
  buildDirectories: [{
    runners: [{
    ...
    mount: {
      mountPath: statePath + '/playground_worker_mount',
      nfsv4: {
        darwin: {
          socketPath: statePath + '/playground_worker_mount.sock',
        },
