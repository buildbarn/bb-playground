		...
		starlark.StringDict{
			"repo": starlark.NewBuiltin("repo", func(thread *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
				...
				newDefaultAttrs, err := getDefaultInheritableAttrs(
					...
					args,
					kwargs,
					model_core.NewSimpleMessage[object.LocalReference](&DefaultInheritableAttrs),
					inlinedTreeOptions,
				)
				if err != nil {
					return nil, err
...
func getDefaultInheritableAttrs(thread *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple, previousInheritableAttrs model_core.Message[*model_starlark_pb.InheritableAttrs, object.LocalReference], inlinedTreeOptions *inlinedtree.Options) (model_core.PatchedMessage[*model_starlark_pb.InheritableAttrs, model_core.CreatedObjectTree], error) {
	if len(args) > 0 {
